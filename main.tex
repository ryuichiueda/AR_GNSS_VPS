...
\title{Recovery from the Failure of LiDAR-based Mobile Robot Localization with the Combination of a Visual Positioning System and a Global Navigation Satellite System}

\author{
\name{M. Yoshigoe\textsuperscript{a},
S. Sasaki\textsuperscript{a},
Y. Hayashibara\textsuperscript{a}
and R. Ueda\textsuperscript{a}\thanks{CONTACT R. Ueda. Email: ryuichi.ueda@chibatech.ac.jp}}
\affil{\textsuperscript{a}Chiba Institute of Technology, 2-17-1 Tsudanuma, Narashino, Chiba, Japan}
}

\maketitle

\begin{abstract}
	This study proposes a method to restore failed LiDAR-based self-localization using global position information from a Visual Positioning System (VPS) or a Global Navigation Satellite System (GNSS). LiDAR-based self-localization methods are widely used in mobile robots. However, once it fails, recovery is difficult. This is because LiDAR data does not provide global positional information. VPS and GNSS, on the other hand, provide global positional information. However, their availability is limited. Moreover, VPS and GNSS are also complementary in that they excel in either densely built-up areas or open areas. In the experiment, we confirmed that our method, by using these three measurement methods complementaryly, significantly improves the probability that the robot can reach its destination.
\end{abstract}

\begin{keywords}
	Grobal navigation satellite system; visual positioning system;  mobile robot localization
\end{keywords}

...
\begin{abstract}
	This study proposes a method to restore failed LiDAR-based self-localization using a Visual Positioning System (VPS) and a Global Navigation Satellite System (GNSS). LiDAR-based self-localization methods are widely used in mobile robots. However, once it fails, recovery is difficult. This is because LiDAR data does not provide global positional information. VPS and GNSS, on the other hand, provide global positional information. However, their availability is limited. Therefore, this research proposes a method to correct a robot using LiDAR-based self-localization when it loses its position by obtaining global positional information from VPS and GNSS. Experiments confirmed that VPS and GNSS complement each other in areas where they are easily accessible, and that this combination can frequently recover from LiDAR-based self-localization failures.
\end{abstract}
